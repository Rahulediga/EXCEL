/**
 * One-time seed script to upload mockData into Firebase Realtime Database.
...
async function seed() {
  ...
  try {
    await set(ref(db, 'wasteComposition'), wasteComposition);
    console.log('  ✅ wasteComposition');

    await set(ref(db, 'weeklyTrend'), weeklyTrend);
    console.log('  ✅ weeklyTrend');

    await set(ref(db, 'monthlyTrend'), monthlyTrend);
    console.log('  ✅ monthlyTrend');

    await set(ref(db, 'bins'), bins);
    console.log('  ✅ bins');

    await set(ref(db, 'alerts'), alerts);
    console.log('  ✅ alerts');

    await set(ref(db, 'recentActivity'), recentActivity);
    console.log('  ✅ recentActivity');

    await set(ref(db, 'reports'), reports);
    console.log('  ✅ reports');

    await set(ref(db, 'predictions'), predictions);
    console.log('  ✅ predictions');

    await set(ref(db, 'recommendations'), recommendations);
    console.log('  ✅ recommendations');

    await set(ref(db, 'patterns'), patterns);
    console.log('  ✅ patterns');

    await set(ref(db, 'analyticsDaily'), analyticsDaily);
    console.log('  ✅ analyticsDaily');

    await set(ref(db, 'collectionEfficiency'), collectionEfficiency);
    console.log('  ✅ collectionEfficiency');

    await set(ref(db, 'dashboardStats'), dashboardStats);
